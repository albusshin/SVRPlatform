...
<%
	...
	try {
		...
		//out.println("<script>alert(\"只允许上传jpg,gif和png类型图片文件\")</script>");
		out.println("wrong type");
		return;
	}
	try{ 
		com.jspsmart.upload.File myFile = mySmartUpload.getFiles().getFile(0);
		if (myFile.isMissing()){
			out.println("<script>alert(\"请先选择要上传的文件\")</script>");
			return;
		}
		else{
			...
			if(file_size<file_size_max){
				...
				//转向servlet处理
				out.println(url+filename+"_min."+ext);
			}
			else{
				//out.println("<script>alert(\"上传文件大小不能超过"+(file_size_max/1024)+"K\")</script>");
